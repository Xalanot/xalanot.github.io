...
Because a <strong>C string</strong> is only an array of characters and an array is implicitly converted to a pointer, we can pass a <string>C string</strong> as a pointer to char. For example we can use a string as an argument for a print function [like this]
```cpp
void print(char* str)
{
  std::cout << str << std::endl;
